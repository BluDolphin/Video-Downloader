...
def download(page: ft.Page):
    ...
    def my_hook(d):
        ...
        if "filename" in d:
            filename = d['filename'].split("\\")[-1].split(".")
        ...
        if d['status'] == 'downloading':
            ...
            if total_bytes and downloaded_bytes:
                completionDecimal = downloaded_bytes / total_bytes
                print(f" - download progress: {completionDecimal:.2f}")
                loadingBar.value = completionDecimal
                downloadingText.value = f"{filename} - {completionDecimal:.2%}"
                page.update()
    ...
    def file_exists(filename):
        return os.path.exists(os.path.join("downloads", filename))
    
    page.controls.clear()
    value = "Starting Download..."
    value2 = ""
    
    downloadingText = ft.Text(value, size=20)
    loadingBar = ft.ProgressBar(width=800, height=10)
    completedText = ft.Text(value2, size=20)
    cancelButton = ft.ElevatedButton("Cancel", on_click=cancelButtonClick)
    
    page.add(ft.Column([downloadingText, loadingBar]))
    page.add(completedText, cancelButton)
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl: 
        try:
            ...
            if file_exists(filename):
                print(f"File '{filename}' already exists in downloads directory. Skipping download.")
                return
...
# https://www.youtube.com/watch?v=_YfqYB-m1AM&list=PL4b6-i5pR3IKwG4X9svp4F8Kf9PuUmM3p
ft.app(main)